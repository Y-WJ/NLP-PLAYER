...
def negative_sample(sample_num,sample_from):
...
#    probabilities[0]+=(1-np.add.reduce(probabilities,0))
    sample=np.random.choice(word,sample_num,replace=True)
    return sample

def generate_batch(word_number,count,batch_size,pin,context_window=1,sample_num=1,sample_window=100):
    ...
    for i in range(batch_size):
        ...
        for j in range(positive_label_num):
            ...
            labels_mark[i][j]=1.
        sample=negative_sample(sample_num-positive_label_num,count[0:sample_window])
