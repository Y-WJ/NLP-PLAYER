...
class CONLL():
    ...
    def generate_batch(self,wv,batch_size=10):
        l=self.length
        vector_size=len(wv['unk'].tolist())
        zero=np.zeros(vector_size,dtype=float).tolist()
        ...
        if not len(stack_square)==batch_size:
            print(pin,len(stack_square),'batch')
        for i in range(stack_len):
            for j in range(batch_size):
                if not len(stack_square[j])==stack_len:
                    print(pin,i,j,'len')
                if isinstance(stack_square[j][i],int):
                    stack_rev[i][j]=zero
                elif stack_square[j][i].decode('utf-8','replace').lower() in wv:
                    stack_rev[i][j]=wv[stack_square[j][i].decode('utf-8','replace').lower()].tolist()
                else:
                    stack_rev[i][j]=unk
        for i in range(buffer_len):
            for j in range(batch_size):
                if isinstance(buffer_square[j][i],int):
                    buffer_rev[i][j]=zero
                elif buffer_square[j][i].decode('utf-8','replace').lower() in wv:
                    buffer_rev[i][j]=wv[buffer_square[j][i].decode('utf-8','replace').lower()].tolist()
                else:
                    buffer_rev[i][j]=unk
        ...
        for i in range(batch_size):
            label_square[i]=tran_embedding[self.tran_dic[label]]
        self.sample_pin=pin
        batch=[stack_rev,buffer_rev,tran_rev,label_square]
        return batch

        '''
    def generate_batch(self,wv,batch_size=10):#this is not in batch
        l=self.length
        tran_embedding=np.eye(self.tran_size).tolist()
        pin=self.sample_pin
        batch=list()
        for i in range(batch_size):
            ...
            if len(sample_list[2])>0:
                ...
                buffer=list()
                tran=list()
                for word in sample_list[0]:
                    if word in wv:
                        stack.append(wv[word].tolist())
                    else:
                        stack.append(wv['unk'].tolist())
                for word in sample_list[1]:
                    if word in wv:
                        buffer.append=(wv[word].tolist())
                    else:
                        buffer.append(wv['unk'].tolist())
                for word in sample_list[2]:
                    if len(word)==2:
                        word=word[0]+word[1]
                    tran.append(tran_embedding[self.tran_dic[word]])
                label=tran[len(tran)-1]
                tran.pop()
                sample=[stack,buffer,tran,label]
                batch.append(sample)
        self.sample_pin=pin
        return batch
    '''
